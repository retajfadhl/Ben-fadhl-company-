...
export default function Hero() {
  ...
  return <section className="relative isolate flex min-h-[700px] items-end overflow-hidden bg-slate-950 pb-16 pt-36 sm:pb-20 lg:min-h-[760px]">
    ...
    <div className="site-shell relative z-10"><div className="max-w-3xl text-white">
      ...
      <div className="mt-14 flex flex-wrap gap-x-7 gap-y-3 border-t border-white/20 pt-6 text-sm font-medium text-slate-200">{(['years', 'operators', 'location'] as const).map(key => <span className="flex items-center gap-2" key={key}><Check size={16} className="text-amber-400" />{t(key)}</span>)}</div>
    </div></div>
  </section>;
}
